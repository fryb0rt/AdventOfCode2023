...
int main()
{
	...
	for (int x = 0; x < lines[0].size(); ++x) {
		sum = std::max(sum, run(Laser{ x,0,2 }, lines));
		sum = std::max(sum, run(Laser{ x,int(lines.size() - 1),3 }, lines));
	}
	for (int y = 0; y < lines.size(); ++y) {
		sum = std::max(sum, run(Laser{ 0, y ,0 }, lines));
		sum = std::max(sum, run(Laser{ int(lines[0].size() - 1), y,1 }, lines));
	}
